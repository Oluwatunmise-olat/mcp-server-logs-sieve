...
export function registerTraceRequest(server: McpServer, provider: LogProvider) {
  server.registerTool(
    "trace_request",
    {
      description:
      ...
      inputSchema: {
        ...
        start_time: z
          ...
          .optional()
          .describe(
            "Start time - ISO 8601 or relative like 1h, 30m (defaults to 1h)",
          ),
        ...
        text_filter: z
          ...
          .optional()
          .describe(
            "Text to search for (e.g. user ID, endpoint, error message)",
          ),
        severity: z
          .enum([
            "DEBUG",
        ...
        resource_type: z
          ...
          .optional()
          .describe(
            "GCP resource type, AWS log group name (required for AWS), Azure table name, Loki stream selector hint, or Elasticsearch index/index-pattern",
          ),
        limit: z
          .number()
          .min(1)
          ...
          .optional()
          .describe(
            "Max seed entries to search for traces (default 50, max 200)",
          ),
      },
    },
    async (params) => {
